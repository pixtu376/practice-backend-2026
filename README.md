...
* **Глубокая аналитика: Автоматический расчет процентного соотношения ответов и подсчет уникальных респондентов.
* **Гибкие списки: Пагинация результатов, фильтрация по владельцу/статусу и сортировка по популярности (количеству ответов).
* **Экспорт данных: Возможность выгрузки полных результатов опроса в формате JSON для внешней обработки.
...
| GET | `/api/surveys` | Список опросов с фильтрацией, сортировкой и пагинацией
| GET | `/api/surveys/{id}/analytics` | Полная статистика по ответам (%, количество, тексты)
| GET | `/api/surveys/{id}/export` | Скачивание результатов опроса в формате JSON-файла
...
4.  **Защита от повторного прохождения: Система проверяет наличие ответов от текущего user_id для конкретного survey_id. При попытке повторной отправки сервер возвращает ошибку доступа.
    > ![Скриншот: Ошибка возвращения](public/Попытка%20повторного%20ответа.png)
---

## Демонстрация расширенных функций

Cистема автоматически агрегирует данные. Для вопросов с выбором рассчитывается доля каждого варианта в процентах, для текстовых — формируется список всех мнений.

> ![Скриншот: аналитики](public/Ананлитика.png)

* Реализована поддержка Query Parameters для управления списками:

* ?filter=my — только созданные текущим пользователем.

* ?status=published — фильтр по состоянию.

* ?sort=answers&direction=desc — сортировка от самых популярных к менее востребованным.

Эндпоинт экспорта формирует JSON-файл с заголовками Content-Disposition, что позволяет скачивать данные напрямую через инструменты или браузер.
